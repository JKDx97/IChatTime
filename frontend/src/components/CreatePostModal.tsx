...
export default function CreatePostModal({ open, onClose, onCreated }: Props) {
  ...
  async function fixImageOrientation(file: File): Promise<string> {
  ...
  async function handleFiles(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < Math.min(selected.length, remaining); i++) {
      ...
      const preview = isVideo ? URL.createObjectURL(f) : await fixImageOrientation(f);
      newItems.push({ file: f, preview, isVideo });
    }
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-lg md:mx-4 animate-slide-up md:animate-modal-enter">
        <div className="rounded-t-2xl md:rounded-2xl bg-white shadow-2xl overflow-hidden max-h-[85vh] md:max-h-none flex flex-col">
          {/* Header */}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                  ...
                  {mediaItems.map((m, i) => (
                    <div key={i} className="relative shrink-0 w-36 h-36 overflow-hidden rounded-xl border border-gray-100">
                      {m.isVideo ? (
                        ...
                        <img src={m.preview} alt={`Vista previa ${i + 1}`} className="w-full h-full object-cover" />
                      )}
                      <button
                        type="button"
            ...
            <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3 shrink-0 safe-area-bottom">
              <button
                type="button"
